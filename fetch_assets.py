...
def make_operation_request(config, *operations):
    operations_xml = "\n".join(f"<Operation>{operation}</Operation>" for operation in operations)
...
def AssetSearchFeaturesForBBOX(source, bbox, feature_types=[]):
    x1, y1, x2, y2 = bbox
    feature_types = "\n".join([f"<FeatureGroupCode>{f}</FeatureGroupCode>" for f in feature_types])
    response = operation_request_as_dict(source, f"""<AssetSearch>
...
def operation_request_as_dict(config, operation):
    ...
    return parsed["{http://schemas.xmlsoap.org/soap/envelope/}Envelope"]["{http://schemas.xmlsoap.org/soap/envelope/}Body"]["{http://www.confirm.co.uk/schema/am/connector/webservice}ProcessOperationsResult"]["Response"]["OperationResponse"]
...
def get_mapit_bbox(area_id, api_key):
    ...
    area = requests.get(f"https://mapit.mysociety.org/area/{area_id}/geometry", headers=headers).json()
    w, s, e, n = int(area['min_e']), int(area['min_n']), int(area['max_e']), int(area['max_n'])
    return (w, s, e, n)
...
def process_layer(layer, config):
    ...
    features = AssetSearchToFeatures(source, bbox, layer['feature_types'], layer['box_size'])

    with fiona.open(os.path.join(OUTPUT_PREFIX, layer['output']), 'w', **meta) as output:
